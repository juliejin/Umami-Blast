...
public class ShapesManager : MonoBehaviour
{
    ...
    private IEnumerator FindMatchesAndCollapse(RaycastHit2D hit2)
    {
        ...
        if(totalMatches.Count() >= Constants.MinimumMatches)
            SpawnFood(totalMatches.First().name);
    ...
    //Checks the prefab name to spawn corresponding model in upper screen
    //Added by Tristan

    public GameObject hamburgerPrefab, topBunPrefab, bottomBunPrefab, slicedCheesePrefab;
    public Vector3 spawnPosition;
    public GameObject ingredientMemory;
    public void SpawnFood(string ingredientToSpawn)
    {

        if(ingredientToSpawn == null)
        {
            print("Null ingredient");
            return;
        }
        
        if(ingredientToSpawn.Contains("Hamburger"))
        {
            // GameObject new3DFood = Instantiate(hamburgerPrefab, spawnPosition, Quaternion.identity);
            ingredientMemory.GetComponent<IngredientMemory>().AddFoodToSpawnedIngredientList("Hamburger");
        }
        if(ingredientToSpawn.Contains("TopBun"))
        {
            // GameObject new3DFood = Instantiate(topBunPrefab, spawnPosition, Quaternion.identity);
            ingredientMemory.GetComponent<IngredientMemory>().AddFoodToSpawnedIngredientList("TopBun");

        }
        if(ingredientToSpawn.Contains("BottomBun"))
        {
            // GameObject new3DFood = Instantiate(bottomBunPrefab, spawnPosition, Quaternion.identity);
            ingredientMemory.GetComponent<IngredientMemory>().AddFoodToSpawnedIngredientList("BottomBun");
        }
        if(ingredientToSpawn.Contains("SlicedCheese"))
        {
            // GameObject new3DFood = Instantiate(slicedCheesePrefab, spawnPosition, Quaternion.identity);
            ingredientMemory.GetComponent<IngredientMemory>().AddFoodToSpawnedIngredientList("SlicedCheese");
        }
        if(ingredientToSpawn.Contains("SegmentCheese"))
        {
            ingredientMemory.GetComponent<IngredientMemory>().AddFoodToSpawnedIngredientList("SegmentCheese");            
        }
    }
